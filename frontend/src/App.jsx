...
function App() {
  ...
  const [showAddFriend, setShowAddFriend] = useState(false); // Controls the modal
  const [newFriendName, setNewFriendName] = useState("");
  const [message, setMessage] = useState("");
  const [chatHistory, setChatHistory] = useState([]);
  
  // Mobile View State: 'list' or 'chat'
  // On desktop, we ignore this and show both.
  ...
  const handleAuth = async (endpoint) => {
    try {
      await axios.post(`http://localhost:5000/${endpoint}`, { username, password });
      if (endpoint === "login") {
        setIsLoggedIn(true);
  ...
  const loadFriends = async () => {
    const res = await axios.get(`http://localhost:5000/my-friends/${username}`);
    setFriends(res.data);
  };

  const addFriend = async () => {
    try {
      await axios.post("http://localhost:5000/add-friend", { myUsername: username, friendUsername: newFriendName });
      setNewFriendName("");
  ...
  return (
    <div className="h-[100dvh] bg-gray-950 flex text-white overflow-hidden relative">
      ...
      {/* SIDEBAR (Friend List) 
          - Hidden on Mobile if we are in 'chat' view
          - Always visible on Desktop (md:flex)
      */}
      <div className={`w-full md:w-1/3 md:max-w-sm bg-gray-900 border-r border-white/5 flex flex-col 
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {friends.length === 0 && (
            <div className="text-center text-gray-500 mt-10">
              <p>No friends yet.</p>
              <p className="text-sm">Tap + to add someone!</p>
            </div>
          )}
          {friends.map((friend) => (
            <motion.div 
              ...
              className={`p-4 rounded-2xl cursor-pointer flex items-center gap-4 transition-all
                ${currentChat === friend ? "bg-white/10" : "hover:bg-white/5"}`}
            >
        ...
        <div className="absolute bottom-6 right-6 md:hidden">
            <button onClick={() => setShowAddFriend(true)} className="w-14 h-14 bg-cyan-500 rounded-full flex items-center justify-center text-black shadow-lg shadow-cyan-500/40">
              <Plus size={28} strokeWidth={3} />
            </button>
        </div>
        {/* Desktop Add Friend Button (at bottom of sidebar) */}
        <div className="p-4 hidden md:block">
           <button onClick={() => setShowAddFriend(true)} className="w-full bg-white/5 hover:bg-white/10 p-3 rounded-xl flex items-center justify-center gap-2 font-medium transition-colors">
             <Plus size={20} /> Add New Friend
           </button>
        </div>
      </div>

      {/* CHAT AREA 
          - Hidden on Mobile if we are in 'list' view
        ...
        {currentChat ? (
          <>
            ...
            <div className="p-4 bg-gray-900/50 backdrop-blur-md border-b border-white/5 flex items-center gap-4 sticky top-0 z-10">
              ...
              <button onClick={() => setMobileView("list")} className="md:hidden p-2 rounded-full hover:bg-white/10">
                <ArrowLeft size={24} />
              </button>
              ...
              <div>
                <h3 className="font-bold text-lg leading-tight">{currentChat}</h3>
              ...
              <AnimatePresence initial={false}>
                {chatHistory.map((msg, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`max-w-[80%] md:max-w-[60%] px-5 py-3 rounded-2xl text-white shadow-sm
                        ${isMe ? "bg-blue-600 rounded-br-none" : "bg-gray-800 rounded-bl-none"}`}>
            ...
            <div className="p-4 bg-gray-900 border-t border-white/5 flex gap-2 items-end">
              <div className="flex-1 bg-gray-800 rounded-2xl flex items-center px-4 py-2 border border-transparent focus-within:border-cyan-500/50 transition-colors">
                <input 
                  className="bg-transparent w-full text-white outline-none py-2 resize-none h-10" 
                  placeholder="Message..."
                  value={message}
              ...
              <motion.button 
                whileTap={{ scale: 0.9 }} 
                onClick={sendMessage} 
                className="bg-cyan-500 p-3 rounded-full text-black shadow-lg shadow-cyan-500/20"
              >
      ...
      {showAddFriend && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div 
            ...
            className="bg-gray-900 border border-white/10 p-6 rounded-3xl w-full max-w-sm shadow-2xl"
          >
            <h3 className="text-xl font-bold mb-4">Add Friend</h3>
            <input 
              autoFocus
              className="w-full bg-black/50 text-white p-3 rounded-xl border border-white/10 mb-4 outline-none focus:border-cyan-500" 
